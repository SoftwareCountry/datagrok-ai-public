...
const RadarChart = function RadarChart(parent_selector, data, options) {
	...
	allAxis.forEach(a => {
		const axisValues = data.map(d => d.axes.find(ax => ax.axis === a)?.value);

		if (axisValues.some(v => typeof v === typeof '')) {
			const comparer = (a, b) =>
				a === null ? -1 : b === null ? 1 : a.localeCompare(b);
			const domain = [...new Set(axisValues)]
			...
			const padding = domain.some(v => v === undefined) ? 0 : 0.2;
			rScales[a] = d3.scalePoint()
				.range([0, radius])
				.domain(domain)
				.padding(padding);
		} else {
			const maxValue = max(cfg.maxValue, maxFromArr(axisValues));
